    ...
    <React.Fragment>
      {overlays.map((overlay, index) => {
        ...
        if (overlay.type === 'polygon') {
          ...
          const { properties: { tappable = false, fillColor, strokeColor, strokeWidth, highlightedFillColor = fillColor } } = overlay.feature
          return (
            <MapView.Polygon
              ...
              onPress={() => {
                if (selectedIndex !== null) {
                  ...
                  if (fillColor && highlightedFillColor !== fillColor) {
                    overlayRefs[selectedIndex].setNativeProps({ fillColor })
                  }
                }
                selectedIndex = index
                if (fillColor && highlightedFillColor !== fillColor) {
                  overlayRefs[selectedIndex].setNativeProps({ fillColor: highlightedFillColor })
                }
              }}
            />
          );
